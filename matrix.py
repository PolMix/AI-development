import sys
sys.path.insert(1, 'D:/Working process/CS/Python Projects/Spyder/LinAlg/lib')
from new_vectors import Vector
...
class Matrix():
    ...
    ''' Decomposes a matrix into N vectors '''
    def decompose(self):
        result = []
        for n in range(0, self.dim_n):
            vector = Vector(self.dim_m)
            vector.set_zeros()
            for m in range(0, self.dim_m):
                vector.vector[m] = self.matrix[m][n]
    ...
    ''' Makes QR decomposition of A matrix '''
    def QR_dec(self):
        if self.dim_m != self.dim_n:
            print("Dimensions are wrong for this method!")
        else:
            ...
            # Gram-Shmidt ortogonalisation
            for n in range(0, D):
                U_vectors[n].replace_vector(A_vectors[n].vector)
                for sum_ind in range(0, n):
                    dot_product = A_vectors[n].dot(E_vectors[sum_ind])
                    times_vector = E_vectors[sum_ind].times(dot_product)
                    U_vectors[n].vector_sum(times_vector, -1)
                norm = U_vectors[n].L2()
                times_vector = U_vectors[n].times(1 / norm)
                E_vectors[n].replace_vector(times_vector.vector)
                # print("Vector U:")
                # U_vectors[n].print_vector()
                # print("Vector E:")
                # E_vectors[n].print_vector()
            
            for n in range(0, D):
                for m in range(0, D):
                    Q.matrix[m][n] = E_vectors[n].vector[m]
                    R.matrix[m][n] = E_vectors[m].dot(A_vectors[n])
    ...
    ''' Finds out eigenvalues based on QR decomposition '''
    def eigenvalues(self):
        if self.dim_m != self.dim_n:
            ...
            while iter_number <= 10:
                iter_number += 1
                QR_result = self.QR_dec()
                QR_result[1].matrix_mult(QR_result[0])
                self.set_matrix_in(QR_result[1].matrix)
            for m in range(0, self.dim_m):    
                result.append(self.matrix[m][m])
